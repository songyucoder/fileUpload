<template>
  <div class="cropper" v-bind="$attrs" :style="{ 'background-color': is_dark ? '#000' : '' }">
    <div
      ref="stage"
      ...
      >
        <div class="face">
          <img
            ...
            alt=""
            :style="{
              transform: `translate3d(${originalImage.left - cropBox.left}px,${
                originalImage.top - cropBox.top
              }px,0) rotateZ(${originalImage.rotate}deg) scale(${originalImage.scale})`
            }"
          />
        </div>
    ...
    <div class="cropper__control">
      <!-- <div class="cropper__icon-rotate" @click="onRotate"></div> -->
    </div>
    <div class="cropper__line"></div>
    <div style="width: 100%; flex-wrap: wrap" class="flex flex-">
      <div
        class="flex mt-5"
        style="width: 25%"
        v-for="(item, index) in setScale_arr"
        @click="seclectRateEvent(item)"
        :key="index"
      >
        <div
          class="flex items-center"
          :class="item.is_select ? 'rate_select' : 'rate_none'"
          style="flex-direction: column; justify-content: center; width: 90%; height: 70px"
        >
          <div
            style="
              ...
              border-width: 0.5px;
            "
            :style="{ width: item.rate1 + 'px', height: item.rate2 + 'px' }"
          ></div>
          <div class="mt-2">{{ item.itmeStr }}</div>
        </div>
      </div>
    </div>
    <div class="cropper__control mt-2">
      <Button class="operate_btn" style="background-color: #f56c6c" @click="onCancel">取消</Button>
      <Button class="operate_btn" style="background-color: #67c23a" @click="onReset">还原</Button>
      <Button class="operate_btn" style="background-color: #409eff" @click="onCrop">选取</Button>
    </div>
  </div>
</template>

<script>
import { reactive, ref, toRefs, onMounted, computed, watch, nextTick } from 'vue'
import {
  getOrientation,
  getImageArrayBuffer,
  dataURLToBlob,
...
export default {
  ...
  setup(props, { emit }) {
    ...
    //  设置寸尺
    const seclectRateEvent = (item) => {
      console.log(item)
      for (const obj of state.setScale_arr) {
        obj.is_select = false
      }
      item.is_select = true
      // 设置裁剪尺寸
      console.log(state.originalImage.width)
      console.log(state.originalImage.height)
      // eslint-disable-next-line vue/no-mutating-props
      const minRate = Math.min(state.originalImage.width, state.originalImage.height)
      console.log(minRate)
      state.cropBox.width = minRate
      state.cropBox.height = minRate * (item.rate2 / item.rate1)
      console.log(state.cropBox)
      console.log(stage.value.clientWidth)
      console.log(stage.value.clientWidth)
...
.rate_select {
  background-color: rgb(11 70 158 / 10%);
  border-color: #39f;
  border-style: solid;
  border-width: 0.5px;
}
